...
def get_ideal_eq():
    ...
    ideal_eq_cmd_arr = np.array([[0, 0, 3, 1],          # X_0; 0
                                 ...
                                 [1, 3, 1, 1],          # C_3; 13
                                 [2, 12, 13, 0]])       # X_0.T @ C_0 @ X_0 + C_1 * X_1 + C_2 * X_1 * X_1 + C_3; 14
    ...
    G=0.3
    H=7
    flattened_params = np.array([(H+G), -H, -G,
                                 -H, (F+H), -F,
                                 -G, -F, (F+G), -198, -9801, -1])
...
def main():
    with open("hill_w_hardening.pkl", "rb") as f:
        x = dill.load(f)

    training_data = ImplicitTrainingDataMD(x)
    N = len(training_data._x)
    ...
    x = [x_0[:N], x_1[:N]]
    training_data._x = x
    training_data._dx_dt = training_data._dx_dt[:N]
    ...
    print("ideal equation explicit fitness:", explicit_fitness(ideal_eq))
    print("before optimize")
    print("clo fitness of ideal_eq:", test_fitness(ideal_eq))
