...
from bingo.symbolic_regression.agraphMD.component_generator_md \
    import ComponentGeneratorMD
from bingo.symbolic_regression.agraphMD.validation_backend.\
...
@pytest.fixture
def fork_agraph():
    test_graph = AGraphMD([(3, 3)], (3, 3))
    test_graph.command_array = np.array([[CONSTANT, -1, 3, 3],  # sin(sin(X_0))
                                         [VARIABLE, 0, 3, 3],
                                         [SUBTRACTION, 1, 1, 3],
                                         [SUBTRACTION, 1, 1, 3],
                                         [SIN, 1, 1, 3],
    ...
    test_graph.set_local_optimization_params(np.ones(9), [(3, 3)])
    return test_graph
...
def not_enough_unutil_fork_agraph():
    test_graph = AGraphMD([(3, 3)], (3, 3))
    test_graph.command_array = np.array([[VARIABLE, 0, 3, 3],  # sin(sin(X_0))
                                         [SUBTRACTION, 0, 0, 3],
                                         [SIN, 0, 0, 3],
                                         [SIN, 2, 2, 3]], dtype=int)
    test_graph.genetic_age = 1
...
def arity_one_op_with_two_different_params_agraph():
    test_graph = AGraphMD([(3, 3)], (3, 3))
    test_graph.command_array = np.array([[VARIABLE, 0, 3, 3],  # sin(X_0)
                                         [SUBTRACTION, 0, 0, 3],
                                         [SUBTRACTION, 0, 0, 3],
                                         [SIN, 0, 2, 3]], dtype=int)
    test_graph.genetic_age = 1
...
def unused_op_invalid_after_fork_mutation_agraph():
    test_graph = AGraphMD([(3, 3)], (3, 3))
    test_graph.command_array = np.array([[VARIABLE, 0, 3, 3],  # sin(sin(sin(X_0)))
                                         [CONSTANT, 0, 3, 3],
                                         [SIN, 0, 0, 3],
                                         [SIN, 2, 2, 3],
                                         [SUBTRACTION, 2, 3, 3],
...
def fork_mutation_component_generator():
    generator = ComponentGeneratorMD(input_x_dimensions=[(3, 3)],
...
def fork_mutation(fork_mutation_component_generator):
    mutation = AGraphMutationMD(fork_mutation_component_generator,
...
def test_fork_mutation(mocker, fork_agraph, fork_mutation, mutation_location):
    ...
    assert validate_individual(child)
...
def test_fork_mutation_generator_has_no_ar1_op(mocker, many_unutil_fork_agraph,
    ...
    mutation = AGraphMutationMD(generator,
                                command_probability=0.0,
                                node_probability=0.0,
                                parameter_probability=0.0,
                                prune_probability=0.0,
