...
from .agraphMD.pytorch_evaluation_backend.evaluation_backend import _get_torch_const, evaluate
...
class ImplicitRegressionMD(VectorBasedFunction):
    ...
    def evaluate_fitness_vector(self, individual):
        ...
        """
        self.eval_count += 1
        _, df_dx = individual.evaluate_equation_with_x_gradient_at(x=self.training_data.x)

        if isinstance(df_dx, list):
            df_dx = np.hstack([element.reshape(element.shape[0], -1) for element in df_dx])

        dot_product = df_dx.squeeze() * self.training_data.dx_dt

        # norm_df_dx = df_dx / np.linalg.norm(df_dx, axis=0)[None, :]
        # norm_dx_dt = self.training_data.dx_dt / np.linalg.norm(self.training_data.dx_dt, axis=0)[None, :]
        # norm_dot_product = norm_df_dx * norm_dx_dt
        # norm_denom = np.sum(np.abs(norm_dot_product), axis=1)
        # norm_fitness = np.sum(norm_dot_product, axis=1)
        # norm_fitness[np.isnan(norm_fitness)] = np.inf
        # return norm_fitness

        # dot_product /= np.linalg.norm(dot_product, axis=1)[:, None]
        # dot_product = np.einsum('ij, ij->i', df_dx.squeeze(), self.training_data.dx_dt)
        # norm_of_dot = np.einsum('ij, ij->i', np.square(df_dx.squeeze()), np.square(self.training_data.dx_dt))
        # return dot_product / norm_of_dot


        if self._required_params is not None:
            if not self._enough_parameters_used(dot_product):
                return np.full((self.training_data.x[0].shape[0],), np.inf)
        ...
        normalized_fitness[np.isnan(normalized_fitness)] = np.inf
...
class ImplicitRegressionMDPytorch(VectorGradientMixin, ImplicitRegressionMD):
    def get_fitness_vector_and_jacobian(self, individual):
        f_of_x, df_dx = individual.evaluate_equation_with_x_gradient_at(self.training_data.x)
        jac_before = individual._simplified_constants[0].grad.view(-1, 9)
        df_dx.requires_grad = True
        fitness_vector = self._fitness_vector(df_dx)
        fitness_vector.sum().backward()

        # dfitness_df_dx = df_dx.grad
        # df_dx_dc = individual._simplified_constants[0].grad
        # dfitness_dc = dfitness_df_dx * df_dx_dc

        jac = individual._simplified_constants[0].grad.view(-1, 9)
        print(np.array_equal(jac_before.detach().numpy(), jac.detach().numpy()))
        return fitness_vector.detach().numpy(), jac.detach().numpy()

    def _fitness_vector(self, df_dx):
        if isinstance(self.training_data.dx_dt, np.ndarray):
            self.training_data._dx_dt = torch.from_numpy(self.training_data.dx_dt).double()

        dot_product = df_dx.squeeze() * self.training_data.dx_dt
        denominator = torch.sum(torch.abs(dot_product), dim=1)
        normalized_fitness = torch.sum(dot_product, dim=1) / denominator
        normalized_fitness[~torch.isfinite(denominator)] = torch.inf
        normalized_fitness[torch.isnan(normalized_fitness)] = torch.inf
        return normalized_fitness

    def evaluate_fitness_vector(self, individual):
        self.eval_count += 1
        _, df_dx = individual.evaluate_equation_with_x_gradient_at(self.training_data.x)

        return self._fitness_vector(df_dx).detach().numpy()


class ImplicitTrainingDataMD(TrainingData):
    """
    ...
    def __getitem__(self, items):
        ...
        ExplicitTrainingData :
            a subset
        """
        temp = ImplicitTrainingDataMD(self._x[items, :], self._dx_dt[items, :])
        return temp
...
def _calculate_partials(X, window_size=7):
