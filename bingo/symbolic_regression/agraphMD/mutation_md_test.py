...
class AGraphMutationMD(Mutation):
    ...
    def __init__(self, component_generator, command_probability=0.2,
        ...
        self._possible_operators = self._component_generator.possible_operators()
    ...
    def _mutate_command(self, individual):
        ...
        dependent_stack = individual.command_array[:mutation_location, :]
        old_command = individual.command_array[mutation_location]
        new_command = \
            self._component_generator.random_command(mutation_location, dependent_stack)
    ...
    def _randomize_node(self, command):
        if IS_TERMINAL_MAP[command[0]]:
            ...
            if command[0] == VARIABLE:
                command[2], command[3] = self._component_generator.input_x_dimensions[command[0]]
            else:  # CONSTANT OR INTEGER
                command[2], command[3] = self._component_generator.random_dims()  # TODO how to deal with constant changing size?
                # TODO need to update constant_shapes of individual somehow (change in _update?)
    ...
    def _get_random_param_mut_location(self, individual):
        ...
        if self._component_generator.num_x <= 1:
    ...
    def _randomize_parameters(self, command, mutation_location):
        if IS_TERMINAL_MAP[command[0]]:
            command[1] = \
                self._component_generator.random_terminal_parameter(command[0])
                # command[2] = command[1]  # TODO how to deal with setting dimension?
        else:
            command[1] = \
